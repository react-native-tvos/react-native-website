...
On Android TV, `eventKeyAction` distinguishes a key going down from a key coming up, so a single button press produces two events. Apple TV does not set this field.
...
`body` is typed loosely because its shape depends on the event. It carries
touch position and velocity for pan gestures on Apple TV, and is absent for
button presses.
